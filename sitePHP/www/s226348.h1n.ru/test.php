...
if($_POST['otkuda'])
  {
    ...
    //  echo "<td>" . $row["id_reis"] . "</td>";
     ...
     echo "</table></div>";
    //  $resultt = $mysqli->query("SELECT marshrut.id, avtobus.avt as id_av  
    //  FROM marshrut
    //  INNER JOIN avtobus ON marshrut.id_av = avtobus.id");
     $resultt = $mysqli->query("SELECT marshrut.id, avtobus.id as id_reis1, avtobus.avt 
     FROM marshrut, avtobus WHERE marshrut.id_av = avtobus.id");
     echo "<div class='tableup'><table class='table' border='1'><tr><th>id</th><th>рейс</th><th>Откуда</th><th>Куда</th><th>Дата отправления</th><th>Дата прибытия</th><th>Цена</th><th>Осталось мест</th><th class='avto'>Авто</th><th class='avto1'></th></tr>";
     
     foreach($resultt as $row3){
     echo "<tr>";
     echo "<td>" . $row3["id"] . "</td>";
     echo "<td>" . $row3["id_reis1"] . "</td>";
     echo "<td>" . $row3["avt"] . "</td>";
